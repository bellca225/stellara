...
void main() {
  group('QuestionRepository', () {
    ...
    test('generates three deterministic local questions for a friend', () async {
      ...
      expect(
        first.map((item) => item.prompt).toSet().length,
        3,
      );
      expect(first.every((item) => item.source == QuestionSource.localPreset), isTrue);
      expect(
