...
class _FriendScreenState extends ConsumerState<FriendScreen> {
  ...
  Future<void> _sendRequest(String toUid, String toNickname) async {
    ...
    if (mounted) {
      ...
      setState(() { _searchResult = null; _searchController.clear(); });
    }
  }
  ...
  Future<void> _toggleFavorite(Friend friend) async {
    ...
    if (friend.isFavorite) {
      ...
      if (favoriteIds.length >= 3) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('즐겨찾기는 최대 3명까지 가능해요.')),
        );
        return;
      }
          ...
          children: [
            ...
            Panel(
              child: Column(
                ...
                children: [
                  const Text('친구 코드로 검색', style: TextStyle(fontWeight: FontWeight.w700)),
                  ...
                  if (_searchResult != null) ...[
                    ...
                    Row(
                      children: [
                        _InitialBadge(initial: (_searchResult!['nickname'] as String? ?? '?')[0]),
            ...
            if (_loading)
              ...
              _friends.isEmpty
                  ? const Panel(child: Center(child: Padding(
                      padding: EdgeInsets.all(24),
                      child: Text('아직 친구가 없어요. 코드로 검색해서 요청해보세요!'),
                    )))
                  : Panel(
                      child: Column(
                        children: [
      ...
      children: [
        ...
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(friend.nickname, style: const TextStyle(fontWeight: FontWeight.w800)),
              Text(friend.friendCode, style: const TextStyle(color: AppColors.inkMuted, fontSize: 13)),
            ],
          ),
        ),
        IconButton(
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        ...
        const SizedBox(width: 12),
        Expanded(
          child: Text(request.fromNickname, style: const TextStyle(fontWeight: FontWeight.w800)),
        ),
        ElevatedButton(
          onPressed: onAccept,
          child: const Text('수락'),
