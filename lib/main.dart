                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (subtitle != null) ...[
                      ...
                      Text(subtitle!, style: const TextStyle(color: _muted, fontSize: 16)),
                    ],
                  ],
                ),
              ),
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: FilledButton.icon(
        ...
        label: const Text('카카오로 시작하기', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w900)),
      ),
    );
  }
}
...
class _Panel extends StatelessWidget {
  const _Panel({required this.child, this.background = _panel, this.padding = const EdgeInsets.all(18)});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: const [BoxShadow(color: Color(0x0D000000), blurRadius: 18, offset: Offset(0, 10))],
      ),
      child: child,
    );
  }
...
class _InputField extends StatelessWidget {
  const _InputField({required this.label, required this.icon, required this.controller});
  ...
  @override
  Widget build(BuildContext context) {
    return TextField(controller: controller, decoration: _inputDecoration(label: label, icon: icon));
  }
}
...
InputDecoration _inputDecoration({required String label, required IconData icon, Widget? suffix}) {
  return InputDecoration(
    ...
    suffixIcon: suffix == null
        ? null
        : Padding(padding: const EdgeInsets.only(right: 14), child: Center(widthFactor: 1, child: suffix)),
    filled: true,
    ...
    focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: _teal, width: 2)),
  );
}

class _IntegrationRow extends StatelessWidget {
  const _IntegrationRow({required this.icon, required this.title, required this.detail});
...
class _PillRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Wrap(spacing: 8, runSpacing: 8, children: items.map((item) => _Pill(item)).toList());
  }
}
...
class _Pill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white, border: Border.all(color: _line), borderRadius: BorderRadius.circular(8)),
      child: Text(text, style: const TextStyle(fontWeight: FontWeight.w700, color: _ink)),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: Stack(
        ...
        children: [
          Container(width: 256, height: 256, decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: _line, width: 2))),
          Container(width: 146, height: 146, decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: _line, width: 2))),
          _OrbitBody(label: centerLabel, size: 82, color: Colors.white, borderColor: _teal),
          Positioned(right: 86, top: 100, child: _OrbitBody(label: partnerLabel, size: 66, color: _paper, borderColor: _coral)),
          Positioned(left: 42, top: 28, child: _PlanetChip(label: '태양', color: _gold, onTap: onPlanetTap)),
          Positioned(right: 34, bottom: 62, child: _PlanetChip(label: '달', color: _coral, onTap: onPlanetTap)),
          Positioned(left: 70, bottom: 30, child: _PlanetChip(label: '수성', color: _teal, onTap: onPlanetTap)),
          Positioned(bottom: 78, child: Text(caption, style: const TextStyle(color: _muted, fontWeight: FontWeight.w800))),
        ],
      ),
    );
  }
...
class _OrbitBody extends StatelessWidget {
  const _OrbitBody({required this.label, required this.size, required this.color, required this.borderColor});
  ...
  Widget build(BuildContext context) {
    return Container(
      width: size,
      height: size,
      decoration: BoxDecoration(color: color, shape: BoxShape.circle, border: Border.all(color: borderColor, width: 2)),
      child: Center(child: Text(label, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w900))),
    );
  }
}

class _PlanetChip extends StatelessWidget {
  const _PlanetChip({required this.label, required this.color, required this.onTap});
...
class _ZodiacAvatar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      width: size,
      height: size,
      decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.white, border: Border.all(color: _line, width: 2)),
      child: Center(child: Text(label, style: TextStyle(fontSize: size * 0.26, fontWeight: FontWeight.w900))),
    );
  }
}
          ...
          children: [
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(friend.name, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w800)),
                  Text('${friend.sign} · ${friend.code}', style: const TextStyle(color: _muted)),
                ],
              ),
            ),
          ...
          Row(
            children: [
              ...
              IconButton(
                ...
                icon: Icon(favorite ? Icons.star : Icons.star_border, color: favorite ? _gold : _muted),
              ),
            ],
          ),
          const SizedBox(height: 10),
          _SecondaryButton(label: '궁합 보기', icon: Icons.favorite_border, onPressed: onMatch),
        ],
      ),
    );
  }
...
class _RequestTile extends StatelessWidget {
  const _RequestTile({required this.name, required this.code, required this.onAccept});
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        ...
        Expanded(child: Text('$name · $code', style: const TextStyle(fontWeight: FontWeight.w800))),
        FilledButton(
          style: FilledButton.styleFrom(backgroundColor: _teal, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
          onPressed: onAccept,
          child: const Text('수락'),
        ),
      ],
...
class _AnalysisCard extends StatelessWidget {
  const _AnalysisCard({required this.title, required this.detail, required this.accent});
    ...
    return Padding(
      ...
      child: _Panel(
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Container(width: 8, height: 58, decoration: BoxDecoration(color: accent, borderRadius: BorderRadius.circular(8))),
...
class _QuestionCard extends StatelessWidget {
  const _QuestionCard({required this.tag, required this.question, required this.accent});
      ...
      child: _Panel(
        ...
        child: Column(
          ...
          children: [
            _Pill(tag),
            const SizedBox(height: 14),
            Text(question, style: const TextStyle(color: _ink, fontSize: 20, fontWeight: FontWeight.w800, height: 1.32)),
          ],
        ),
      ),
    );
...
class _MoodRow extends StatelessWidget {
  const _MoodRow({required this.icon, required this.label, required this.detail});
      ...
      child: _Panel(
        child: Column(
          ...
          children: [
            ...
            ClipRRect(
              borderRadius: BorderRadius.circular(8),
              child: LinearProgressIndicator(value: score / 100, minHeight: 10, backgroundColor: const Color(0xFFE6ECEA), color: color),
            ),
          ],
        ),
      ),
...
class Friend {
  const Friend({required this.id, required this.name, required this.sign, required this.code});
