...
3. **client_secret이 앱 바이너리에 포함**되는 구조입니다(현재 9주차 단계 결정). README와 코드 주석 모두 “릴리즈 단계에서는 백엔드 프록시로 분리 권장”을 명시. SDD에는 “Phase 2에 백엔드 프록시 도입”을 비기능 요구사항으로 못 박는 편이 좋습니다.
4. **Prokerala sandbox 제약**: README에 “현재 sandbox 계정은 1월 1일만 허용 → 다른 날짜 입력 시 fixture 차트로 자동 fallback”이라고 명시되어 있어 데모 시나리오 작성에 영향 줍니다.
