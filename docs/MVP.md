...
> **작성 기준**: SDD v0.2 + 코드베이스 실측 (2026-05-08)  
...
| M7 | AI 랜덤 질문 3개 생성 | CONTENT-001 | Cloud Functions → OpenAI → 질문 3개 화면 표시 + 사용자 질문 1개 입력 |
...
| S5 | AI 성격 리포트 (장문) | MYPAGE-001 또는 ASTROLOGY-001 | OpenAI 기반 1,000자 이상 분석. Cloud Functions 확장으로 구현 가능 |
...
| 자체 백엔드 REST 서버 | Firebase + Cloud Functions으로 완전 대체 가능 |
| 결제/구독 기능 | 범위 외 |
...
| **P0** | Cloud Functions 프록시 (Prokerala natal/horoscope/synastry) | API 키 보호 + 모든 점성술 기능의 전제 조건 |
...
Step 2.  Cloud Functions 초기화 (functions/ 폴더)
         → places-resolve, prokerala-natal, prokerala-horoscope 프록시 작성
         → Firebase 환경변수에 PROKERALA_CLIENT_ID/SECRET 설정
         → 기존 ProkeralaApi 클래스 → Functions 호출로 교체

Step 3.  Firestore users / charts / friendCodes 설계 확정 + 팀원 4명 테스트 데이터 시딩
         → Firebase Console에서 직접 입력

Step 4.  places-resolve 연결
         → 온보딩 출생지 입력 → 실제 lat/lng 저장
         → BirthInfo 는 dateTimeLocal + utcOffset 형태 유지

Step 5.  LoginScreen에 "개발 모드 진입" 버튼 추가
         → uid 하드코딩 또는 입력 → Firestore 조회 → AppShell 진입
...
Step 10. MATCH-001 실 Synastry 데이터 연결
         - Cloud Functions prokerala-synastry 호출
...
```
Step 11. Cloud Functions ai-questions 추가 (OpenAI 연동)
         - CONTENT-001에 Functions 호출 연결
...
Stellara는 **출생 정보(생년월일·시간·장소) → Prokerala API → 나탈 차트·운세·궁합** 흐름으로 동작하는 Flutter 앱이다. 모든 점성술 계산은 외부 API에 위임하고, 앱은 시각화·소셜 기능을 담당한다. 백엔드는 Firebase(Firestore + Cloud Functions)로 구성되며, API 키는 모두 Cloud Functions 서버에 격리되어 있다.
...
functions/        → Firebase Cloud Functions (10주차부터 존재)
...
4. **Prokerala sandbox 제약**: 현재 sandbox 계정은 **1월 1일 날짜만 허용**한다. 다른 날짜를 입력하면 앱이 자동으로 fixture 차트로 fallback한다. 이는 데모 시 주의가 필요하다.

5. **DB 시딩**: 카카오 로그인이 없는 동안 `LoginScreen`의 "개발 모드 진입" 버튼을 통해 Firestore에 직접 입력된 테스트 uid로 진입한다.
...
- [ ] Cloud Functions 배포 완료 (`places-resolve`, `prokerala-natal`, `prokerala-horoscope`)
